...
public class LightningChainMaker : UdonSharpBehaviour
{
    ...
    [SerializeField] Vector3PairList segments;
    ...
    private void GenerateLightningBolt(Vector3 start, Vector3 end, int generation, int totalGenerations, float offsetAmount)
    {
        ...
        segments.Add(start,end);//.Add(new KeyValuePair<Vector3, Vector3>(start, end));
        if (generation == 0)
        {
            return;
        }
        ...
        while (generation-- > 0)
        {
            ...
            {
                start = segments.GetKey(i);
                end = segments.GetValue(i);
                ...
                segments.Add(start, midPoint);
                segments.Add(midPoint, end);
            }
    ...
    public void RandomVector(ref Vector3 start, ref Vector3 end, float offsetAmount, out Vector3 result)
    {
        if (orthographic)
        {
            ...
            float distance = (RandomFloat() * offsetAmount * 2.0f) - offsetAmount;
        ...
        else
        {
            ...
            float distance = ((RandomFloat() + 0.1f) * offsetAmount);

            // get random rotation angle to rotate around the current direction
            float rotationAngle = (RandomFloat() * 360.0f);
    ...
    private void SelectOffsetFromAnimationMode()
    {
        ...
        {
           index = UnityEngine.Random.Range(0, offsets.Length);
        }
    ...
    private void UpdateLineRenderer()
    {
        ...
        int index = 0;
        lineRenderer.SetPosition(index++, segments.GetKey(startIndex));

        for (int i = startIndex; i < segments.Count; i++)
        {
            lineRenderer.SetPosition(index++, segments.GetValue(i));
        }
